...
| Backend Development | 🔨 Identity module delivered (Phase 6) — no other business features yet |
| Frontend Development | 🔨 Auth UI delivered (Phase 6) — no other business features yet |
| Mobile Development | 🔨 Auth wired into both apps (Phase 6) — no other business features yet |
| Testing | 🔨 Harness in place — **359 tests pass** (259 backend + 56 frontend across 7 Nx projects + 44 Flutter across 7 packages/apps), all re-run fresh at Phase 6 close-out — see [`planning/features/06-authentication-authorization/STATUS.md`](../planning/features/06-authentication-authorization/STATUS.md) |
...
**Completed:** Phase 6 — Authentication & Authorization (2026-08-10)

**Next:** Phase 7 — Administration & Tenant/Master Data (**not started**; requires explicit go-ahead). WebSocket connection/subscription authorization (excluded from Phase 3's `RealtimePublisher`) is tracked as its own immediate fast-follow, not bundled into a later numbered phase.

Phase 6 delivered a complete, hand-built Identity module across all three stacks: JWT (RS256, `pyjwt[crypto]`) + Argon2id password auth, OTP login for Customer/Driver, RBAC (claims-based + live DB re-query for four high-sensitivity actions), password reset — under strict per-tenant RLS via narrowly-scoped `SECURITY DEFINER` PostgreSQL functions (ADR-035, solving the "look up a user before any tenant context exists" chicken-and-egg problem RLS creates). Replaces Phase 2's interim `HeaderTenantResolver`; closes DW-12. Frontend got shell-bypass routing (ADR-036, `/login` renders outside the authenticated shell) and its first Reactive Forms feature library. Mobile got new `api_client` (ADR-037, hand-written, explicit revisit trigger) and `auth` packages, with OTP sign-in and `go_router` guards wired into both apps. A critical, previously-latent FastAPI `Depends()`-resolution bug was found and fixed along the way. Full detail: [`planning/features/06-authentication-authorization/STATUS.md`](../planning/features/06-authentication-authorization/STATUS.md).
...
| Module | Status |
|----------|--------|
| Identity & Access | 🔨 Authentication delivered (Phase 6) — user-management CRUD is Phase 7 |
...
7. **Administration & Tenant/Master Data (Phase 7)** — recommended next; the roadmap's next dependency now that real Authentication exists.
...
- Unit of Work, one illustrative repository/CQRS use case, and the domain-event dispatcher are now implemented (Phase 2) — no business aggregate, repository, or router beyond Identity (Phase 6) exists yet.
