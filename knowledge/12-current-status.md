...
| Cylinder Ledger | 🟡 In progress (Phase 13) — backend complete & verified, frontend partial |
...
|------|---------|-------------|
| 2026-08-13 | 1.15 | **Phases 8-13 committed** — six phases of finished work had been sitting entirely uncommitted (last commit was Phase 7), now in 10 reviewable commits grouped by feature area. **Phase 13 (Cylinder Ledger) backend verified and fixed**: its STATUS.md claimed the backend done; the gates and integration suite disproved it — the fourth false-completion claim in this project after Phases 8, 9 and 12. Seven defects fixed, incl. a `typing.Protocol` instantiated as a class (crashed every order delivery), a handler in the application layer importing api + infrastructure (two merge-blocking contract breaks), a double-subscription that would have double-counted every cylinder on the customer ledger, an `async` repo method declared sync and never awaited (ledger never persisted at all), and a migration with **no RLS and no grants** — no tenant isolation on customers' cylinder balances. Backend now green: ruff, `mypy --strict`, 5/5 import-linter contracts, 251 integration + 494 unit tests. Grants/RLS migration applied to both the test DB and Supabase dev. **Still pending on Phase 13**: the frontend ledger tab compiles but `feature-customers` imports it `type:feature`→`type:feature`, failing `nx lint`; plus no projection tests, no component tests, and no live verification. |
