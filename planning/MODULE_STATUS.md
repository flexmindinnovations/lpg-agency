...
| 20 | Regulatory & MDG Compliance | *(new)* | 🔵 planned, one exception | 0%\* | — | — | Hard requirements — weighment, TDT rating, cylinder identity, DAC, vouchers, PAHAL/PMUY, compliance calendar, cash settlement. Confirmed 2026-08-24: zero backend routers exist for this (`ls backend/src/lpg/api/v1/routers/` — 20 real routers, none of them this). Not a dashboard gap, a whole-stack one. **\*Exception, 2026-09-07:** D24 (driver/vehicle compliance-document pack — one line item of this row's much larger backlog, not the row's other 7 items) is ✅ built and independently re-verified: standalone `ComplianceDocument` aggregate + RLS'd table + CRUD/verify endpoints (`e19c33e`, `16337f4`, `953f41e`, `fd2ccaf`), a shared `lpg-compliance-documents-panel` in both Driver/Vehicle detail drawers plus an Expiry filter on both list pages, live-verified end-to-end in-browser (`0dcd374`), and a nightly `check_compliance_expiry` notification cron, live-verified end-to-end against the dev DB/Redis (`a30f4e1`). See `planning/features/20-regulatory-compliance/STATUS.md` and ADR-038. The other 7 backlog rows (weighment, TDT, DAC, vouchers, PAHAL/PMUY, the rest of the compliance calendar, cash settlement) remain untouched — this is a narrow exception, not a status change for the row as a whole. |
