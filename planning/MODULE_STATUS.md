...
| 19 | Customer app V2 | In Progress — feature-complete, pending live verification | 🟡 `flutter analyze` clean + widget tests pass | ~90% | ✅ (CI gates) | — | ~40 dart files. All 11 TASKS.md items implemented as of 2026-08-31: orders/tracking/invoices/complaints/addresses/KYC-submission/payments + WebSocket live updates + **FCM push** (backend `4bbe6d6`, client `0de0dee`). **Android build now needs `android/app/google-services.json`** (accepted trade-off) — so the newest changes are verified by analyze + widget tests only, not on a device. `device_token` migration `f3a9c1e07b42` not yet applied to a live DB. Feature-screen test coverage still thin (kyc + payments have widget tests; orders/invoices/complaints don't). See `planning/features/19-customer-app-v2/STATUS.md`, C13 |
...
| 20 | Regulatory & MDG Compliance | *(new)* | 🔵 planned, six exceptions | 0%\* | — | — | Hard requirements — weighment, TDT rating, cylinder identity, DAC, vouchers, PAHAL/PMUY, compliance calendar, cash settlement. Confirmed 2026-08-24: zero backend routers exist for this (`ls backend/src/lpg/api/v1/routers/` — 20 real routers, none of them this). Not a dashboard gap, a whole-stack one. **\*Exception, 2026-09-07:** D24 (driver/vehicle compliance-document pack — one line item of this row's much larger backlog, not the row's other items) is ✅ built and independently re-verified: standalone `ComplianceDocument` aggregate + RLS'd table + CRUD/verify endpoints (`e19c33e`, `16337f4`, `953f41e`, `fd2ccaf`), a shared `lpg-compliance-documents-panel` in both Driver/Vehicle detail drawers plus an Expiry filter on both list pages, live-verified end-to-end in-browser (`0dcd374`), and a nightly `check_compliance_expiry` notification cron, live-verified end-to-end against the dev DB/Redis (`a30f4e1`). See ADR-038. **\*Exception, 2026-09-07 (Weighment):** MDG cl. 1.2/1.4 goods-receipt/load-out weighment (Scale registry, batch-level `WeighmentRecord`, tenant-opt-in 100% load-out gate) is ✅ built and independently re-verified — `b3a6fc1`→`a650ac1`, ADR-039 (batch not per-serial) + ADR-040 (opt-in gate, found via a full backend regression that an unconditional gate broke 3 pre-existing tests). **\*Exception, 2026-09-08 (TDT):** the quarterly star-rating mechanism (domain module, two-pass repository query, API, report tab, tenant-config sub-form) is ✅ built and independently re-verified — `90b9a7f`→`ed9f62d`, ADR-041; the mechanism only — real MDG band thresholds/fine percentages remain unconfirmed placeholders pending OMC/tenant sign-off. **\*Exception, 2026-09-08 (Cylinder Identity):** per-cylinder identity + custody + condition lifecycle, and the Rule-26 due-for-test receive() block, is ✅ built and independently re-verified — `2e9b302`→`81f0e62`, ADR-042; the mechanism only — the real IS 15975 statutory retest interval is unconfirmed (every `test_due_date` is staff-entered), and bulk-flow (GRN/load/delivery) integration is explicitly deferred, not built. **\*Exception, 2026-09-08 (DAC):** Delivery Authentication Code capture on proof of delivery, distinct from the platform's own internal delivery OTP, across backend + dashboard + the Flutter driver app's online and offline-queue paths, is ✅ built and independently re-verified — `69883e7`→`3cfce39` (+ this docs commit), ADR-043; a `dac_coverage` reporting KPI is explicitly deferred, not built. **\*Exception, 2026-09-08 (Compliance Calendar):** the Tier-0 licence registry — PESO Form F per warehouse and the tenant's own insurance policy — extending `ComplianceDocument` to two new owner types (`warehouse`, `tenant`) rather than a new aggregate, is ✅ built and independently re-verified — `af0b64d`→`1a5285e` (+ this docs commit), ADR-044; biennial safety inspection (D13), the 12-hour accident-notification workflow (R5/R18), and a calendar-completeness KPI are explicitly deferred, not built. See `planning/features/20-regulatory-compliance/STATUS.md` for all six. The remaining backlog (vouchers, PAHAL/PMUY, biennial safety inspection, the accident-notification workflow, cash settlement) remains untouched — these are narrow exceptions, not a status change for the row as a whole. |
| 21 | AI Foundation | *(new)* | 🔵 planned, one exception | 0%\* | — | — | Model gateway, feature store, evaluation harness, guardrails, kill switch. **\*Exception, 2026-09-09:** the Model Gateway's provider-agnostic tool-calling port + Gemini adapter + tenant kill switch + budget ceiling + append-only run ledger is ✅ built and independently re-verified — `6fd078d`→`aba1698`, ADR-045; live-verified end to end with a real Gemini API key (genuine multi-tool orchestration, real data matching the dashboard). A real config-value string/boolean bug (`bool("false") is True` in Python) was caught during live verification and fixed, with the same risk named as still-latent in `weighment_gate_enabled` (ADR-040), not fixed there. Feature store, evaluation harness, inference serving, and full guardrails/governance remain untouched. See `planning/features/21-ai-foundation/STATUS.md`. |
...
| 23 | AI Assistive Interfaces | *(new)* | 🔵 planned, one exception | 0%\* | — | — | LLM/hybrid — complaint triage, KYC extraction, conversational ordering, voice, analytics copilot, MDG copilot. **\*Exception, 2026-09-09:** a narrower, safer precursor to A12 (analytics copilot) — a fixed-tool-menu "AI Command Center" instead of text-to-SQL over a semantic layer — is ✅ built and independently re-verified, same commits as row 21's exception (`6fd078d`→`aba1698`, ADR-045); the real A12 design, and A5/A6/A7/A8/A11/A13, all remain untouched. See `planning/features/23-ai-assistive-interfaces/STATUS.md`. |
...
| 26 | Agency Provisioning (Super Admin "Create agency") | *(new, not in original roadmap)* | ✅ backend, ⚠️ UI | 100% | ⚠️ | — | Built 2026-09-25 (ADR-048, `planning/features/30-agency-provisioning/`). `POST /platform/agencies` creates a tenant plus its first `agency_admin` and returns a one-time password-setup link (email delivery is not wired up, so the Super Admin relays it). Tenant creation goes through a new `SECURITY DEFINER` function (`tenant.tenant_provision`) because `tenant.tenant`'s RLS forbids INSERT; the audit row is written by hand because the audit hook cannot see a function call. **Verified by re-running the gates:** ruff, mypy strict, import-linter (6 contracts), 1,025 unit tests, 6 new integration tests against real PostgreSQL plus 175 related integration/tenant-isolation tests, and a live HTTP walkthrough (create → 409/422/401 cases → set password via the link → link is single-use → login blocked `LICENSE_NOT_ACTIVATED` → issue+activate license → new admin logs in with `agency_admin` + 56 permissions → 403 on `/platform/*`). **Not verified:** the Create-agency drawer has 16 passing component tests but was never exercised in a real browser. **Missing:** subdomain routing (slug is validated as a DNS label but nothing resolves it), email delivery, an authenticated change-password endpoint. |
| 27 | Agency User Management (Super Admin recovery + extra admins) | *(new, not in original roadmap)* | ✅ | 100% | ✅ | — | Built 2026-09-26 (ADR-049, `planning/features/31-agency-user-management/`). Found in production: the Create-agency setup link was single-use and expired after 1 hour, with no email provider and no change-password endpoint, so a missed link locked an agency out with no recovery path. Three Platform Console routes: list an agency's users, add an `agency_admin`, issue a fresh setup link (24 h, single-use; older unused links invalidated by a new `SECURITY DEFINER` function). Explicit audit entries attributed to the target agency (the audit hook cannot see these writes), never containing a token. **Verified by re-running the gates:** ruff, mypy strict, import-linter (6 contracts), OpenAPI drift, 1,039 unit tests, 7 new integration tests against real PostgreSQL (RLS 404 for another agency's user, old link dead after re-issue, audit attributed to the agency) plus 182 related integration/isolation tests, a 15-check live HTTP walkthrough, frontend 23 page tests + 53 data-access tests, lint, production build, and the UI **exercised in a real browser** (users list, add admin, validation, duplicate-email error toast, new setup link, and Create agency through the shared drawer). **Not verified:** production deployment of this change; light theme and narrow/mobile widths of the new Users block. **Known gaps:** using a reset link does not revoke the user's existing sessions (predates this); links are still returned in the response and logged (no email provider); deactivating an agency user from the Platform Console is not included. |
