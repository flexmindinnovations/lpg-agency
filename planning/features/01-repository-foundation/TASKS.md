...
- [x] **T-68** PrimeNG installation & integration (ADR-028's approved hybrid strategy — PrimeNG as primary component library — was decided but never installed). **COMPLETE 2026-08-09.**
  *Installed:* `primeng@22.0.0` + `@primeuix/themes@3.0.0` + `primeicons@8.0.0` + `@angular/animations@~22.0.4` (peer-matched to installed Angular `~22.0.4`/CDK `^22.1.1`, verified against the npm registry, not assumed).
  *Token integration:* `LpgPrimeNgPreset` (`libs/shared/design-tokens/src/lib/primeng-preset.ts`) — a `definePreset(Aura, {...})` where every value is `var(--token-name)` or a `color-mix()` derivative; no second design system, no hardcoded component colours. `transitionDuration` maps to `--motion-duration-small`, which `tokens.css`'s `prefers-reduced-motion` block already collapses to near-zero. `providePrimeNG()` wired in `apps/dashboard/src/app/app.config.ts`, dark-mode selector matched to `ThemeService`'s own `[data-theme]` attribute.
  *Discovered-and-fixed bug (pre-existing, not introduced by this task):* `styles.css` and the AG Grid wrapper (`data-grid.component.ts`) referenced `var(--semantic-color-*)` / `var(--semantic-spacing-*)` / `var(--semantic-radius-*)` / `var(--semantic-border-width)` — variables the token generator has never actually emitted (it strips the `semantic` group's prefix to nothing; real names are the bare `--color-*` / `--spacing-*` / `--radius-*` / `--border-width`). Introduced in the original Phase 1 commit (`470436e`); every one of those `var()` references had been silently resolving to nothing (transparent background, inherited/initial text colour) since Phase 1 was marked complete — it happened to look plausible in light theme by coincidence. Confirmed with the user before touching already-completed Phase 1 files, then fixed both (mechanical rename, no logic change); re-verified light/dark/high-contrast rendering and AG Grid's own test suite afterward.
  *Accessibility fix:* PrimeNG's `p-dialog` has no built-in focus-return-to-trigger behaviour (checked `primeng-dialog.d.ts` — no such input exists). Added an explicit `viewChild` + `(onHide)` handler in `home.ts` so WCAG 2.2 AA focus return (D-35) holds; verified Escape closes the dialog and focus lands back on the triggering button.
  *Licence key:* never hardcoded (same rule as `AG_GRID_LICENSE_KEY`, ADR-020). `apps/dashboard/src/app/prime-license.ts` is git-ignored (`.gitignore`: `frontend/apps/dashboard/src/app/prime-license.ts`); `prime-license.example.ts` is the committed template. `app.config.ts` imports `PRIME_NG_LICENSE_KEY` from the git-ignored file and passes it to `providePrimeNG({ license: ... })` — absent, PrimeNG simply runs unlicensed (its own banner), never a build failure.
  *Verified:* `nx build dashboard` (536.39kB initial, under the raised 550kB budget — accepted as a real, expected cost, not hidden), `nx lint` and `nx test` across `dashboard`, `shared-design-tokens`, `shared-ui` (AG Grid wrapper's 9 tests unaffected), `prettier --check`, design-token drift check (`node scripts/generate-tokens.mjs --check`), light/dark/high-contrast theme rendering in a live browser, dialog focus-trap/Escape/focus-return, breadcrumb/tabs/select/toast/tooltip rendering.
