...
**2026-08-24 — read this note before the rest of this document.** Everything below is historical, largely unmaintained since Phase 13 (2026-08-13) — the roadmap table further down still says Phases 14–19 are "Not started," which was already wrong before this note was added. **[`planning/MODULE_STATUS.md`](./MODULE_STATUS.md) is the actively-maintained, gate-verified status doc and wins on any disagreement with this file** (this file says so itself, further down — see "Remaining documentation gaps"). As of 2026-08-24, `MODULE_STATUS.md` covers 25 modules including two not in this document's roadmap at all: a **License** module (commit `f902ced`, 2026-08-23) and a **Platform Console** / `super_admin` control plane (commit `9fdb3c9`, 2026-08-24). The same session also found and fixed a completely non-functional Complaint Management frontend (see `MODULE_STATUS.md`'s C14) that every automated gate had been reporting green — worth reading if you're about to trust a green CI run as proof a feature works.
...
**Phase 19 — Customer App V2** 🔄 **IN PROGRESS** — Planning professional-level features (Support, KYC, Addresses, Push Notifications, Invoicing, Live Tracking) with a Bottom Navigation Bar shell.

**Recommended next:** Mobile Application Development (Phase 19 execution).
...
Its STATUS.md initially marked every backend item done. Running the gates and the integration suite disproved that — **the fourth time in this project a completion claim has failed independent verification** (Phases 8, 9, 12 preceded it). Seven defects were found and fixed in `8637c9a`, the most serious being: a `typing.Protocol` instantiated as if it were a class, crashing *every* order delivery; a handler in the application layer importing both the api and infrastructure layers, breaking two merge-blocking contracts; a subscription to two events that both fire from one delivery, which would have **double-counted every cylinder** on the customer's ledger; an `async` repository method declared sync and never awaited, so **the ledger was never actually persisted**; and a migration with **no RLS and no grants**, leaving no tenant isolation on a table holding every customer's outstanding cylinder balance. Backend passes ruff, `mypy --strict`, all 5 import-linter contracts, and 251 integration + 494 unit tests. The grants/RLS migration (`a7c2e91b5d84`) is now applied to **all four** environments.

**Environment parity (2026-08-18).** All four databases — `lpg_dev`, `lpg_test`, `lpg_uat` and Supabase — sit at head `e2a91c4f7b58` and return zero defects from `backend/scripts/verify_env_parity.sql` across 36 tenant-scoped tables. Getting there exposed one process bug and three more instances of the recurring isolation-wiring defect:

- **The silent-target bug.** `migrations/env.py::_database_url()` reads `os.environ` only and never loads `backend/.env`, so a bare `alembic upgrade head` fell through to its `localhost:55432/lpg_dev` fallback — reporting a successful upgrade against local dev while appearing to target Supabase. This is what produced the earlier false "applied to Supabase" claims. `env.py` now prints the resolved host and its source on every run. Supabase was 21 revisions behind; `lpg_uat` was 28.
- **`complaint` + `notification` (`c5e83f1a7b96`)** — five tables with RLS enabled but not FORCEd, non-null-safe predicates, and grants that either hardcoded `lpg_app` (so `lpg_uat` got nothing) or were missing entirely.
- **`delivery.driver` / `delivery.vehicle` (`d9f47a2c8e13`)** — predicates calling `current_setting` *without* `missing_ok`, so an unscoped connection raised `unrecognized configuration parameter` rather than returning no rows.
- **`identity.identity_user_permission` (`e2a91c4f7b58`)** — a literal `GRANT ... TO lpg_app`; since `lpg_app` is cluster-wide the statement succeeded on `lpg_uat` while granting the wrong role.

Verified end to end as the application role: with tenant context set, all seeded rows are readable; with another tenant set, zero rows; with no context at all, zero rows rather than an error.
...
2026-08-24 — License module (`f902ced`, 2026-08-23) and Platform Console/`super_admin` control plane (`9fdb3c9`) verified and landed; neither is reflected in this document's roadmap table above — see [`planning/MODULE_STATUS.md`](./MODULE_STATUS.md) rows 24–25 for the authoritative record, including real bugs found and fixed in both (Dispatch driver/vehicle name resolution, every invoice PDF crashing on generation, an `authInterceptor` swallowing session-expiry errors, a guard-ordering race on hard reload, an `import-linter` regression). Same session then live-tested every module in the Angular Dashboard end to end and found Complaint Management's frontend was **completely non-functional** — wrong API host for every request, a button whose imports were commented out, a dead grid-click handler, a permission code that doesn't exist in the database — despite every automated gate (ruff, mypy, `nx lint`, `nx test`, `nx build`) reporting green the entire time. See `MODULE_STATUS.md`'s C14 for the full writeup; it's a genuinely new failure mode for this project's "verify, don't trust" discipline, since this is the first instance the automated gates themselves could not have caught. Also closed two silent gaps (`/admin/employees` and `/notifications` existed and worked but had no sidebar nav link) and built the previously-entirely-missing Cash Handover UI (`POST /cash-handovers` had zero frontend consumption before this).
