...
**Phase 13 — Cylinder Ledger** 🟡 **IN PROGRESS** — started 2026-08-13 by a session running in parallel with this one, discovered mid-session when `libs/ledger/feature-ledger/` appeared in the working tree. **Backend is now complete and independently verified**; frontend is partial. Full record: [`planning/features/13-cylinder-ledger/STATUS.md`](./features/13-cylinder-ledger/STATUS.md).

Its STATUS.md initially marked every backend item done. Running the gates and the integration suite disproved that — **the fourth time in this project a completion claim has failed independent verification** (Phases 8, 9, 12 preceded it). Seven defects were found and fixed in `8637c9a`, the most serious being: a `typing.Protocol` instantiated as if it were a class, crashing *every* order delivery; a handler in the application layer importing both the api and infrastructure layers, breaking two merge-blocking contracts; a subscription to two events that both fire from one delivery, which would have **double-counted every cylinder** on the customer's ledger; an `async` repository method declared sync and never awaited, so **the ledger was never actually persisted**; and a migration with **no RLS and no grants**, leaving no tenant isolation on a table holding every customer's outstanding cylinder balance. Backend now passes ruff, `mypy --strict`, all 5 import-linter contracts, and 251 integration + 494 unit tests. The grants/RLS migration (`a7c2e91b5d84`) is applied to both the local test DB and the Supabase dev DB.

**What's pending on Phase 13** (detail in its STATUS.md): the frontend ledger panel renders as a tab on Customer detail and compiles, but `feature-customers.ts` imports it directly — a `type:feature` library depending on another `type:feature` library — which **fails `nx lint` today**. That needs a design decision (extract a `type:ui` library, or give the ledger its own route), not a tag edit. Also outstanding: no use-case or integration tests for the event projection, no component tests, no live verification, and this phase's own `PLAN.md`/`TASKS.md` still describe the original intent rather than what was built.

**Recommended next:** close out Phase 13's frontend before starting Phase 14 (Accounting & Billing) — Accounting depends on the ledger, and `nx lint` is red until the boundary violation is resolved.
