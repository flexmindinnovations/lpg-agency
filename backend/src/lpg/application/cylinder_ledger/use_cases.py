...
class AppendLedgerTransactionsFromOrderUseCase:
    ...
    async def execute(
        ...
        *,
        ...
        # 4. Save the Ledger (The Unit of Work handles the commit/save in practice,
        # but the repository might need an explicit add if it was newly created)
        if ledger.version == 1 and not ledger.pending_transactions:
            pass # No transactions and new, nothing to save
        else:
            self._ledger_repository.add(ledger)
