...
def upgrade() -> None:
    ...
    if "employee_id" not in columns:
        op.add_column(
            "driver", sa.Column("employee_id", sa.Uuid(), nullable=True), schema="delivery"
        )
        # Because there could be existing drivers, we can't easily map them to
        # non-existent employees in this migration without losing data. But
        # since this is development, we will just delete all drivers to make
        # it NOT NULL, or just create dummy employees? Easiest is to delete
        # all existing drivers and route assignments for safety in this early
        # phase, or set it nullable=True for now. The requirement says "add
        # employee_id (foreign key reference)", and the model says
        # `nullable=False`. Let's clean up existing delivery data since this
        # is a hard breaking change.
    ...
    if "employee_code" in columns:
        op.drop_constraint(
            "uq_driver_tenant_employee_code", "driver", schema="delivery", type_="unique"
        )
...
def downgrade() -> None:
    ...
    op.create_unique_constraint(
        "uq_driver_tenant_employee_code",
        "driver",
