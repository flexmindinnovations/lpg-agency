    ...
    op.execute(
        ...
        WITH RankedDeliveries AS (
            SELECT
                ...
                LAG(lt.performed_at) OVER (
                    PARTITION BY lt.tenant_id, cl.customer_id ORDER BY lt.performed_at
                ) as prev_time
        ...
        SELECT
            tenant_id,
            customer_id,
            AVG(EXTRACT(EPOCH FROM (transaction_time - prev_time))/86400.0)
                as avg_refill_interval_days
