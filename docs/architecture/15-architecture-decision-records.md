...
## ADR-046: Zero-Click Driver Auto-Assignment — Deterministic Scoring, Not an AI Tool, Opt-In Kill Switch, No New System-Principal Bypass

**Status:** Accepted

**Context:** The user asked for order-to-delivery fulfillment automation — the moment an order is confirmed, automatically assign an available driver in the right region, notify everyone, and let the order flow through with zero dispatcher clicks. Research surfaced a real tension worth deciding explicitly, not silently resolving either way: driver assignment today is entirely manual (a plain, unfiltered dropdown), and this project's own prior planning documents state the same guardrail principle three separate times — ADR-045 ("no approval/risk engine is needed *yet*, because no tool can write"), `21-ai-foundation/PLAN.md` §5 ("nothing... may take an irreversible action — no auto-cancel, no auto-credit-denial, no auto-complaint-closure"), and `23-ai-assistive-interfaces/PLAN.md`'s opening rule ("the model never decides anything that carries money, entitlement, or compliance weight... every state change goes through the existing use cases... exactly as if a human had typed it into the UI"). Shown this tension directly (quoting the above) against two more conservative alternatives — suggest-and-confirm, and hybrid auto-only-when-unambiguous — the user chose full autonomy: always zero-click, no human confirmation step. This ADR records that choice and the mitigations shipped alongside it, matching the shape every prior "opt out of the default-safe path" feature in this codebase has used (`weighment_gate_enabled`, ADR-040; `ai_gateway_enabled`, ADR-045): an explicit, default-off, per-tenant toggle — never silently defaulted to on.

**Decision (deterministic scoring, not an LLM/AI tool):** `SuggestDriverAndVehicleForOrderUseCase` (`application/delivery/auto_assignment.py`) is plain filtering and sorting — same branch as the order, `status == "active"`, no currently-open route, tie-broken by today's existing stop count — never a model call. This directly follows the same guardrail principle the user was shown before choosing full autonomy: picking the least-loaded idle driver is a data query, not a judgment call an LLM should make, and a deterministic function is testable, auditable, and carries no hallucination risk. The AI Command Center (ADR-045) is untouched by this feature — no new tool was added to `TOOL_REGISTRY`, and this use case is invoked directly by a background job, never through the model gateway.

**Decision (event-driven trigger, reusing two already-proven patterns rather than inventing a third):** `Order.confirm()` already fires `BookingConfirmed` — no new domain event was needed. A new handler (`infrastructure/events/auto_assignment_handlers.py`) enqueues a new ARQ job (`auto_assign_driver`, `infrastructure/jobs/auto_assignment_jobs.py`) with a deterministic `_job_id` (ARQ's own dedupe primitive — the same pattern `check_compliance_expiry` already uses for "a retried or overlapping trigger enqueues the job at most once"). The job itself calls the exact same `AssignOrderUseCase` a human's click already uses — no parallel mutation path was built. This mirrors the existing `bulk_cancel_orders` job's own precedent: a background job calling a real mutating use case directly, with no principal, no `require_permission` check in the call path — that pattern already existed and works in this codebase; this feature reuses it rather than building a new bypass.

**Decision (no new system-principal/RBAC concept):** research confirmed no system/service-actor concept exists anywhere in this codebase — use cases take no `principal` argument at all, only plain scalar attribution fields, and permission enforcement lives solely at the FastAPI router layer. Rather than inventing a new bypass mechanism, this feature reuses the one that already exists (background jobs calling use cases directly) and solves attribution with a fixed, well-known sentinel actor id (`AUTO_ASSIGNMENT_ACTOR_ID`, `infrastructure/jobs/auto_assignment_jobs.py`) plus a `user_display_name` of `"System (Auto-Assignment)"` threaded through `RequestTenantContext`. No migration was needed for this — confirmed directly against the models that no FK constraint exists on any attribution column (`OrderModel.changed_by`, `AuditLogModel.actor_id`), so the existing Audit Log admin page shows the sentinel actor's display name correctly with zero frontend change.

**Decision (a Postgres advisory lock closes a real, verified gap, not an assumed one):** the two-orders-one-driver race was originally expected to be caught by this codebase's existing optimistic-concurrency machinery — verified directly against every relevant model and it does not actually exist at the ORM layer (`version` is a plain, unenforced integer column; no model sets `__mapper_args__ = {"version_id_col": ...}`; no `with_for_update` exists anywhere in this codebase). `auto_assign_driver` closes this gap explicitly with `pg_advisory_xact_lock(hashtext(driver_id))`, scoped to the job's own transaction and auto-released on commit/rollback, followed by a freshness re-check before the actual assignment call. The dead `version` column itself is named as pre-existing, unrelated debt — flagged, not fixed, as part of this feature.

**Decision (the kill switch, and every failure path, favor the untouched manual flow over a stuck order):** `auto_assignment_enabled` (new `TenantConfiguration` key, default-off) is checked with `is_truthy_config_value()` — promoted out of `application/ai/use_cases.py` (ADR-045) into `application/common/config.py` now that this feature is a second real consumer of the same jsonb-string-`"false"` handling that key's own bug fix already established. Every exit path in the job — disabled tenant, order already assigned by a human, no eligible candidate, the driver taken by a concurrent job inside the advisory lock — is a logged, safe no-op that leaves the order exactly as reachable as it was before the job ran (`confirmed`, ready for the existing manual assign flow), never a crash, never a stuck or half-mutated order. The no-eligible-candidate path additionally enqueues a new `order_unassignable_staff` in-app notification (branch-scoped via the existing `EmployeeBranchStaffResolver`, the same resolution `delivery_failed_staff` already uses) — the same "ranked queue for human review" pattern this codebase already uses for diversion detection (`22-ai-operational-intelligence/PLAN.md` Slice D: "No auto-suspension, ever"), applied to this feature's own failure path rather than its happy path.

**Consequences:**
- **Auto-assignment is genuinely the exception to this codebase's own repeated guardrail principle, not an extension of it** — recorded here explicitly rather than left implicit, so a future reader understands this was a deliberate, informed choice (the user was shown the tension and chose it) and not an oversight. Every mitigation above exists specifically because this feature's default state is "acts without a human," unlike every AI-adjacent feature that came before it.
- **The Dispatch Board "Auto-assigned" badge was scoped out.** `RouteStopResponse` — what the Dispatch Board actually renders — is built via `model_validate()` at eleven separate call sites in `routers/route.py` with no shared response-building helper; adding a computed field would mean touching all eleven, a materially larger change than originally scoped. The existing Audit Log admin page already shows the sentinel actor with zero changes (verified live), so dispatchers are not actually blind to which assignments were automated — deferred, not silently dropped, and named as a candidate follow-up if a shared route-response helper is ever built for other reasons.
- **Two more gaps were found and deliberately left unfixed, named rather than silently worked around**: the dead `version`/optimistic-concurrency column (real, pre-existing, unrelated to this feature beyond motivating the advisory-lock mitigation), and the lack of a bulk driver/vehicle-availability query (`SuggestDriverAndVehicleForOrderUseCase` accepts one query per candidate driver/vehicle rather than a single bulk `LEFT JOIN` — accepted for now on the same "infrequent call, realistic branch sizes" reasoning `GetTodayDeliveryStatusUseCase` already uses, ADR-045; revisit if branch sizes grow).

**Alternatives Considered:**
- **Suggest-and-confirm (a ranked suggestion surfaces on the Dispatch Board, one click to confirm)** — the option that best matches this codebase's own stated guardrail philosophy, and the one this ADR's own Context section recommended; not chosen. The user explicitly opted for full autonomy after being shown the tradeoff.
- **Hybrid: auto-assign only when exactly one eligible candidate exists, otherwise suggest-and-confirm** — a middle ground threading between the literal request and the existing guardrail principle; also not chosen, for the same reason.
- **An LLM-driven assignment tool added to the AI Command Center's `TOOL_REGISTRY`** — rejected outright, independent of the autonomy-level question: picking a driver by branch/idleness/load is a deterministic data query, and routing it through a model would add latency, cost, and a hallucination surface for a decision that needs none of the reasoning an LLM provides.
- **A new system-principal/JWT-minting concept for the background actor** — rejected: the existing `bulk_cancel_orders` job already proves "a background job calls a mutating use case directly, no principal, attribution via a plain scalar field" works in this codebase; inventing a parallel authentication concept for one feature would be new, untested surface for no benefit the existing pattern doesn't already provide.
