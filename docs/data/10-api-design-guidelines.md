...
- Complex/combined filters via a whitelisted filter-expression subset for AG Grid's "saved filters" feature (an Enterprise-tier capability, enabled per ADR-028 only where a feature documents the need) — never an unbounded arbitrary expression tree.
