...
## 6. PrimeNG and Angular Material
**PrimeNG** is the primary component library (ADR-028), used for the bulk of form controls, overlays, and navigation components in the shared UI library. **Angular Material** is used selectively, only where it offers a superior primitive or integration not otherwise covered by PrimeNG or CDK directly. Both libraries' visual styling is fully overridden by the design token system (`09-design-tokens.md`) via their respective theming APIs — PrimeNG via `@primeuix/themes`, Material via its own CSS custom properties — neither ships its default visual language, which would conflict with the platform's Linear/Stripe-benchmarked visual identity.
