...
- **Data Grid:** AG Grid Enterprise, **only through the shared wrapper components** in `libs/shared/ui`. Feature libraries must never import AG Grid types or call its APIs directly (ADR-020). The licence key comes from environment configuration and is never committed.
